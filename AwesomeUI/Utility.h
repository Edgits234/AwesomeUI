...
void display_info(const char* input, int i, int iend = -1)
{
  ...
  extern char *__malloc_heap_end; // end of RAM based on the compiler
#endif

int getRamFree()
{
    //create variable on the stack to figure out where the stack ends
    char topOfStack;
    
    if(__brkval == 0)
    {
        return &topOfStack - __malloc_heap_start;
    }else
    {
...
#define TOTAL_RAM ((size_t)(RAMEND) - (size_t)(RAMSTART) + (size_t)(1))
